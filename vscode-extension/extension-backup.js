...
async function activate(context) {
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand('tiaportal-mcp.addMcpConfig', () => addMcpConfig(context)),
    ...
    // Add/update MCP configuration automatically on activation
    await addMcpConfig(context);
    ...
    logInfo('MCP server configuration added/updated in workspace .vscode/mcp.json');
}
...
// MCP Server Configuration Management (for GitHub Copilot integration via mcp.json)
async function addMcpConfig(context) {
    try {
        ...
        if (!workspaceFolder) {
            logWarning('No workspace folder found. MCP configuration not added.');
            return;
        }
        ...
        if (fs.existsSync(mcpConfigPath)) {
            try {
                ...
                logWarning(`Error reading existing MCP configuration: ${error.message}`);
                mcpConfig = { mcpServers: {} };
            }
        }
        ...
        try {
            ...
            logInfo(`✅ MCP server "${serverName}" configured in: ${mcpConfigPath}`);
            vscode.window.showInformationMessage(`TIA Portal MCP server configured in workspace!`);
        } catch (error) {
            logError(`Error writing MCP configuration: ${error.message}`);
            throw error;
        }
    } catch (error) {
        logError(`Failed to add MCP configuration: ${error.message}`);
    }
}
...
function removeMcpConfig() {
    try {
        ...
        if (!fs.existsSync(mcpConfigPath)) {
            logInfo('MCP configuration file does not exist.');
        ...
        try {
            ...
            if (mcpConfig.mcpServers && mcpConfig.mcpServers['tiaportal-mcp-v19']) {
                delete mcpConfig.mcpServers['tiaportal-mcp-v19'];
                fs.writeFileSync(mcpConfigPath, JSON.stringify(mcpConfig, null, 2), 'utf8');
                logInfo('✅ MCP server removed from configuration');
                vscode.window.showInformationMessage('TIA Portal MCP server removed from workspace configuration!');
            }
        } catch (error) {
            logWarning(`Error removing MCP server: ${error.message}`);
        }
    } catch (error) {
        logError(`Failed to remove MCP configuration: ${error.message}`);
    }
}
