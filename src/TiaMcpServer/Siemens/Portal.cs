...
namespace TiaMcpServer.Siemens
{
    public class Portal
    {
        ...
        {
            _logger?.LogInformation($"Creating new project: {projectName} at {projectPath}");
            ...
            try
            {
                ...
                // Create new project using ProjectComposition.Create
                _project = _portal?.Projects.Create(projectDirectory, projectName);

                _logger?.LogInformation($"Project created successfully: {_project?.Name}");
                ...
                try
                {
                    ...
                    if (result != null)
                    {
                        foreach (var message in result.Messages)
                        {
                            ...
                            _logger?.Log(loglevel, $"Compiler message: {message.State} - {message.Path} - {message.Description}");
                        }
                    }
