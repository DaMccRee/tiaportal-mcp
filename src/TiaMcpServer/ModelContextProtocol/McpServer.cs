...
namespace TiaMcpServer.ModelContextProtocol
{
    ...
    public static class McpServer
    {
        ...
        public static ResponseCreateProject CreateProject(
            [Description("projectPath: defines the directory path where the project will be created")] string projectPath,
            [Description("projectName: defines the name of the new project")] string projectName)
        {
            try
            {
                ...
                {
                    throw new McpException("Project path cannot be empty", McpErrorCode.InvalidParams);
                }

                if (string.IsNullOrWhiteSpace(projectName))
                {
                    throw new McpException("Project name cannot be empty", McpErrorCode.InvalidParams);
                }
                ...
                // Create the project
                bool success = Portal.CreateProject(projectPath, projectName);
            ...
            [Description("password: the password to access adminsitration, default: no password")] string password = "")
        {
            try
            {
                ...
                if (result != null)
                {
                    ...
                    {
                        var messageState = msg.State.ToString();
                        ...
                        if (messageState == "Error")
                            errorCount++;
                        else if (messageState == "Warning")
                            warningCount++;
                    }
                    
                    var stateStr = result.State.ToString();
                    var success = !stateStr.Equals("Error");
                    
                    return new ResponseCompileSoftware
                    {
                        Message = $"Software '{softwarePath}' compiled - State: {stateStr}, Errors: {errorCount}, Warnings: {warningCount}",
                        State = stateStr,
                ...
                {
                    throw new McpException($"Failed compiling software '{softwarePath}': result is null", McpErrorCode.InternalError);
                }
            }
            catch (Exception ex) when (ex is not McpException)
            {
